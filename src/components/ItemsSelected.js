...
function ItemsSelected() {
 const {cardList,setCardList}= useContext(StorContext)
 let newCart=[] 
...
const [bool,setBool]=useState(true)
 ...
 newCart=cardList.filter((product)=>+e.target.value!==product.id)
 console.log(newCart);
  ...
  return (
    <div className='list-item'>
      {cardList.map((item)=> <li class="list-group-item">
          ...
          <div class="total-cart">Total price   :   <span className='some' > €{Math.ceil(total-check)}</span></div>
  </div>
  )
}
